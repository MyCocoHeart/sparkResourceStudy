...
def portable_hash(x):
	...
	这个函数返回用于构建类型一致的散列值，特别None和有None的元祖tuple。
...
def _parse_memory(s):
	...
	把一个内存字符串解析为Java支持的格式解(例如:1g，200m) 返回值为的单位为MB
...
def ignore_unicode_prefix(f):
	...
	在测试文档中忽略字符串的“u”前缀，以使其可以工作在python2和3中
    """
    if sys.version >= '3':
        # the representation of unicode string in Python 3 does not have prefix 'u',
        # so remove the prefix 'u' for doc tests
		#在Python 3中，unicode字符串的表示没有前缀'u'，因此，为测试文档中要移除前缀'u'
...
class RDD(object):
    ...
    """
	"""
	弹性的分布式数据集(RDD)，是Spark的基本抽象对象。表示的是一个不可变的、分区的且可以并行操作的元素集合。
	"""
    ...
    def __getnewargs__(self):
		...
		#这种方法在尝试pickle RDD时被调用，这通常是一个错误
        raise Exception(
    ...
    def persist(self, storageLevel=StorageLevel.MEMORY_ONLY):
		...
		设置rdd的存储级别，且在第一次设置以后这个值将一直被保持。
		如果rdd没有设置存储级别则它仅仅只能被用来设置一个新的存储级别。
		如果没有特别指出使用哪种存储级别，则默认使用{MEMORY_ONLY}
    ...
    def checkpoint(self):
		...
		将此RDD标记为检查点。
		它将被保存到L {SparkContext.setCheckpointDir（）}设置的检查点目录中的文件中，并且对其父RDD的所有引用都将被删除。 
		该方法必须在此RDD上执行任何作业之前调。强烈建议将此RDD保存在内存中，否则将其保存在文件中时需要重新计算。
        """
    ...
    def flatMap(self, f, preservesPartitioning=False):
        ...
        RDD, and then flattening the results.
		 首先将一个函数应用于所有元素，返回一个新的RDD，然后展开结果并合并。
    ...
    def sample(self, withReplacement, fraction, seed=None):
        ...
        :param fraction: expected size of the sample as a fraction of this RDD's size
            ...
            without replacement: probability that each element is chosen; fraction must be [0, 1]
				 不放回采样：每个元素被选中的概率; 概率必须是[0，1]
            with replacement: expected number of times each element is chosen; fraction must be >= 0
				放回采样：每个元素被选择的预期次数; 概率必须> = 0
    ...
    def takeSample(self, withReplacement, num, seed=None):
        ...
        .. note:: This method should only be used if the resulting array is expected
			...
			这个方法只能用于预期的结果数组很小的情况下，因为所有数据都被加载到驱动程序的内存中。
    ...
    def _computeFractionForSampleSize(sampleSizeLowerBound, total, withReplacement):
		...
		令p = num / total，其中num是样本大小，total是rdd中数据总数
        """
    ...
    def intersection(self, other):
		...
		返回此RDD与另一个rdd的交集。即时输入的rdd有重复元素，输出也将不包含任何重复的元素。
    ...
    def repartitionAndSortWithinPartitions(self, numPartitions=None, partitionFunc=portable_hash,
		...
		根据给定的分区程序重新将RDD分区，并在每个结果分区中，按键来排序记录
    ...
    def glom(self):
        ...
        into a list.
		返回通过合并每个分区内的所有元素到一个list所构成的RDD。
    ...
    def collect(self):
		...
		返回包含此RDD中所有元素的list列表

        .. note:: This method should only be used if the resulting array is expected
            to be small, as all the data is loaded into the driver's memory.
			这个方法只能用于预期的结果数组很小的情况下，因为所有数据都会被加载到驱动程序的内存中
        """
    ...
    def fold(self, zeroValue, op):
		...
		汇总每个分区的元素，然后对汇总后的结果使用一个给定的关联函数和一个中性的“零值”
    ...
    def aggregate(self, zeroValue, seqOp, combOp):
        ...
        value."
		汇总每个分区的元素，然后汇总后的结果分区，使用给定的组合函数和一个中性的“零”
        值。”
