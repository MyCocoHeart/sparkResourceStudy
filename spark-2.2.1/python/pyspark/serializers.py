...
class Serializer(object):
    ...
    def _load_stream_without_unbatching(self, stream):
        """
		param:stream 输入流
		func:返回来自输入流的对象的反序列化批(可迭代)的迭代器
			 如果序列化程序不做批处理操作，则默认实现将返回单个元素列表的迭代器
			 即:返回一个迭代器,它是调用了一个其值在输入迭代器上的函数, 返回结果
...
class FramedSerializer(Serializer):
	...
	将Serializer对象写成一种(长度，数据)的成对格式，其中长度是一个32位整数，而数据是C{length}字节。
    """
...
class BatchedSerializer(Serializer):
    ...
    def _batched(self, iterator):
		"""
		param:iterator 迭代器
		return：把迭代器元素分块并转化为list
		参考：hasattr():函数用于判断对象是否包含对应的属性
...
class AutoBatchedSerializer(BatchedSerializer):
    ...
    def dump_stream(self, iterator, stream):
		"""
		序列化对象小于最佳块大小（64k）,如果小于64k则分为2块，如果块大小大于640k,且块数>1则分为int（batch/2）块
		参考:tertools.islice(iterable, start, stop[, step])创建一个迭代器，生成项的方式类似于切片返回值：
			iterable[start : stop : step]，将跳过前start个项，迭代在stop所指定的位置停止，step指定用于跳过项的步幅。
...
def write_int(value, stream):
	"""
	向当前流中写入字节序列，并将此流中的当前位置提升到写入的字节数位置(追加方式)
	参考：struct.pack(fmt, v1, v2, ...)，参数fmt是格式字符串，v1, v2, ...表示要转换的python值
	"""
