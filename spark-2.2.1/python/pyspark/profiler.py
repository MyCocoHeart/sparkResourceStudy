...
class ProfilerCollector(object):
	...
	这个类在每个基础的stage中跟踪不同的剖析器。这也被用于在不同的stage创建新的剖析器。
    """
    ...
    def add_profiler(self, id, profiler):
        ...
        if not self.profilers:
            if self.profile_dump_path:
                atexit.register(self.dump_profiles, self.profile_dump_path)#用于注册程序退出时的回调函数，然后在回调函数中做一些资源清理的操作
            else:
