...
class DataFrame(object):
    ...
    A :class:`DataFrame` is equivalent to a relational table in Spark SQL, ‘DataFrame’类相当于Spark SQL中的一个关系表，
    and can be created using various functions in :class:`SQLContext`::它可以 通过`SQLContext`类中的各种方法创建

        people = sqlContext.read.parquet("...")

    Once created, it can be manipulated using the various domain-specific-language 一旦它被创建以后，可以通过各种语言的方法来操作它
    (DSL) functions defined in: :class:`DataFrame`, :class:`Column`.

    To select a column from the data frame, use the apply method::要从数据框架中选择一个列，请使用apply方法
