...
object SparkSubmit extends CommandLineUtils {
   ...
   *使用REST协议请求现有提交的状态。独仅适合独立和Mesos的集群模式。
   */
  ...
  private[deploy] def prepareSubmitEnvironment(
    ...
    if (clusterManager == YARN) {
      ...
      // Make sure YARN is included in our build if we're trying to use it如果我们要使用yarn那么我们需要确保我们的build中包含了它
    ...
    // Update args.deployMode if it is null. It will be passed down as a Spark property later.如果是空，则更新arg.deploymode。稍后它将作为Spark属性传递。
    ...
    // too for packages that include Python code如果有任何并添加类路径到jars，就解决maven依赖关系。将它们添加到py-files ，对于包含Python代码的包也是如此
    ...
    // Create the IvySettings, either load from file or build defaults创建IvySettings，要么从文件中加载，要么构建默认值
	...
	//通过使用--jars 或者 --packages.安装任意R包
    if (args.isR && !StringUtils.isBlank(args.jars)) {
      RPackageUtils.checkAndBuildRPackage(args.jars, printStream, args.verbose)
    }

    // assure a keytab is available from any place in a JVM 确保在JVM中的任何位置都可以使用keytab
